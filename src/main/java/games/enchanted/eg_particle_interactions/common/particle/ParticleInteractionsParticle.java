...
public class ParticleInteractionsParticle extends Particle {
    ...
    public void extract(QuadConsumerProvider quadConsumerProvider, Camera camera, float partialTicks) {
        ...
        double lerpedY = (float) Mth.lerp(partialTicks, this.yo, this.y) + this.randomRenderOffset;
...
//            Gizmos.billboardText(this.onGround ? "g" : "", new Vec3(lerpedX, lerpedY, lerpedZ), TextGizmo.Style.whiteAndCentered());
    ...
    protected void doCollisions() {
        ...
        this.downCollision = this.collide(
            xVel * 2,
            // this is so particles don't get stuck floating if the collision below gets removed
