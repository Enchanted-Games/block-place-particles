...
public class QuadFaceShape {
     ...
     */
    public void renderShapeWithRotation(VertexConsumer vertexConsumer, Vector2f[] uvCoordinates, Vector3f pos, Vector3f scale, Quaternionf facingCameraQuaternion, Vector3f rotation, float size, int lightColour, Vector4f colour) {
        ...
        for (int i = 0; i < this.vertices.length; i += 4) {
            Vector3d vertex1 = facingCameraQuaternion.transform( MathHelpers.rotate3DPoint( new Vector3d(this.vertices[i]    ).mul(scale), pitchRad, yawRad, rollRad ) ).mul(size).add(pos.x, pos.y, pos.z);
            Vector3d vertex2 = facingCameraQuaternion.transform( MathHelpers.rotate3DPoint( new Vector3d(this.vertices[i + 1]).mul(scale), pitchRad, yawRad, rollRad ) ).mul(size).add(pos.x, pos.y, pos.z);
            Vector3d vertex3 = facingCameraQuaternion.transform( MathHelpers.rotate3DPoint( new Vector3d(this.vertices[i + 2]).mul(scale), pitchRad, yawRad, rollRad ) ).mul(size).add(pos.x, pos.y, pos.z);
            Vector3d vertex4 = facingCameraQuaternion.transform( MathHelpers.rotate3DPoint( new Vector3d(this.vertices[i + 3]).mul(scale), pitchRad, yawRad, rollRad ) ).mul(size).add(pos.x, pos.y, pos.z);
    ...
    /**
     * @see QuadFaceShape#renderShapeWithRotation(VertexConsumer, Vector2f[], Vector3f, Vector3f, Quaternionf, Vector3f, float, int, Vector4f)
     *
     */
    public void renderShape(VertexConsumer vertexConsumer, Vector2f[] uvCoordinates, Vector3f pos, Vector3f scale, Quaternionf facingCameraQuaternion, float size, int lightColour, Vector4f colour) {
        this.renderShapeWithRotation(vertexConsumer, uvCoordinates, pos, scale, facingCameraQuaternion, new Vector3f(0), size, lightColour, colour);
    }
